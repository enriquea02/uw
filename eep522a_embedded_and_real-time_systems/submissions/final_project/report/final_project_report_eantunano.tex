...
\begin{document}
    ...
    The ILDA standard specifies that the input voltage to an ILDA motor driver should be +/- 5V.
    I was only able to achieve a voltage range of 0V - 3.1V with a precision of $\div{1}{4096}$ from the 12-bit DAC output.
    The DAC was configured to its max gain output $2 * V_{REF}$, so I would not be able to achieve a range of 0V to 5V.
    Thus, I setup two bipolar op amp circuits, one for the x-axis and another for the y-axis, which would allow be to adjust the gain and offset of the ILDA motor driver input.
    
    The bipolar op amp was designed in two stages.
    First stage was for the gain, while the second stage was for the offset.
    Separating the circuit into two stages allowed for easy adjustments and modifications to the final circuit as necessary.
